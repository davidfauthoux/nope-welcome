...
import { Server, uuid, history } from "../modules/server-ext.js";
...
// $.getJSON("data.json" , function(data) {
	...
	let superuserUser = platform + "/superuser";
	...
	async.run([
		...
		() => {
			...
			if (userId === null) {
				...
				return async._([
					...
					(address) => {
						...
						userId = platform + "/" + emailAddress;
					},
					async.try_([
						() => encryptionServer.getPublicKey(userId),
						(publicKey) => { console.log("USER PUBLIC KEY", publicKey); },
						() => encryptionServer.recoverUser(userId, emailSubjectText(), userData.language, supervise),
			...
			userData.email = userId.substring(userId.indexOf('/') + 1);
